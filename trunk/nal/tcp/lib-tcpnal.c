 ...
 * The NAL is asynchronous and thread-safe.
 *
 * If PTL_PROGRESS_THREAD is defined at compile time, a progress POSIX
...
/* define this to use semaphore instead of mutex+condition */
/* #undef EVENT_SEMAPHORE */
#define EVENT_SEMAPHORE

#ifdef PTL_PROGRESS_THREAD
#  include <pthread.h>
#  ifdef EVENT_SEMAPHORE
#    include <semaphore.h>
#  endif /* EVENT_SEMAPHORE */
...
/* This NAL is used in user space, and services a single Portals-using
 * process. A NAL is uniquely identified by a (NID, PID) pair.
 */
...
struct p3tcp_data {
	...
	/* channels waiting for connection to succeed */
	struct list_head conn_wait;

	/* We need to know our PID before we can start up, but we can't look
	 * it up via our p3_process_t since the NI isn't installed there until
	 * the NAL instance has successfully started.
	 * Grrrrrrr.
	 */
	ptl_pid_t pid;
...
int p3tcp_my_nid(const char *if_str, unsigned int *nid)
{
...
int p3tcp_listen(p3tcp_data_t *d)
{
	...
	/* Well-known PIDs have assigned ports, so see if we have a port other
	 * than the default.
	 */
 ...
 */
void p3tcp_drain_chan(p3tcp_data_t *d, lib_ni_t *ni)
{
	unsigned i;
...
#if !defined PTL_PROGRESS_THREAD
	p3tcp_chan_poll(d, 0);
#endif
	p3_lock(&d->dlock);
...
/* lies! it used to busy wait! ;) */
#ifdef PTL_PROGRESS_THREAD
static volatile int eq_poll_cond_init = 0;
#  ifndef EVENT_SEMAPHORE
static pthread_mutex_t eq_poll_cond_lock;
static pthread_cond_t eq_poll_cond;
static volatile int eq_poll_event_pending = 0;
#  else
static sem_t eq_poll_semaphore;
#  endif /* EVENT_SEMAPHORE */
#endif /* PTL_PROGRESS_THREAD */

void p3tcp_chan_poll(p3tcp_data_t *d, ptl_time_t timeout)
{
...
again:
	 ...
	 */
	nr = select(nfds, &rfd_set, &wfd_set, NULL, to);
	if (nr < 0) {
		if (errno == EINTR)
			goto again;
		err_abort(errno, "p3tcp_chan_poll: select failed");
	}
	else if (nr == 0) {
#ifdef PTL_PROGRESS_THREAD
		err_abort(errno, "p3tcp_chan_poll: select returned 0");
#else
		return;
#endif /* PTL_PROGRESS_THREAD */
	}
...
#ifdef PTL_PROGRESS_THREAD
	/* this is butt-ugly, but is a remnant of the old code; if configured
	   to run with a user progress thread, do not return, regardless of
	   timeout, until the thread is signalled to stop.
	   the butt-ugliness stems from the fact that semantics of the
	   function silently changes, and that the rest of the code must
	   asure that it will never be called, except from the progress
	   thread */
	if (stop) {
		if (DEBUG_NI(d,PTL_DBG_NI_06))
		...
		if (DEBUG_NI(d,PTL_DBG_NI_06))
			p3_print("signaling eq_poll_cond (pending=%d)\n",
				 eq_poll_event_pending);
		pthread_cond_broadcast(&eq_poll_cond);
		pthread_mutex_unlock(&eq_poll_cond_lock);
#else
		if (DEBUG_NI(d,PTL_DBG_NI_06))
			p3_print("signaling semaphore\n");
		sem_post(&eq_poll_semaphore);
#endif /* EVENT_SEMAPHORE */
		/* there is some strange race condition in the MT code, causing
		   mayhem in event passing between API and NAL: after a few 10
		   or a few 100 events, the API does not see a new one, and
		   communcation stalls; it occurs with condition and with a
		   semaphore, so it must not be POSIX threads related;
		   if we sleep for 10ms at this point the race seems to go
		   away */
		/* usleep(10000); */
		goto again;
	}
...
#ifdef PTL_PROGRESS_THREAD
	p3tcp_chan_poll(nal_data, PTL_TIME_FOREVER);
	/* when configured with user progress thread, p3tcp_chan_poll() will
	   not return until the thread is signalled to stop */
#endif
	return NULL;
}
...
void p3tcp_send_start(p3tcp_data_t *d, lib_ni_t *ni)
{
	 ...
	 */
	p3tcp_chan_poll(ni->nal->private, 0);
#endif
	if (DEBUG_NI(d,PTL_DBG_NI_06))
...
static
int p3tcp_progress(lib_ni_t *ni, ptl_time_t timeout)
{
	p3tcp_data_t *d = ni->nal->private;
#if !defined PTL_PROGRESS_THREAD
	p3tcp_chan_poll(d, timeout);
#else /* defined PTL_PROGRESS_THREAD */
	struct timeval tv0;
	struct timespec ts;
	int twr;
...
#else
		sem_init(&eq_poll_semaphore, 0, 0);
#endif /* EVENT_SEMAPHORE */
	}
	...
	while (twr != ETIMEDOUT && eq_poll_event_pending == 0) {

		if (DEBUG_NI(d, PTL_DBG_NI_06))
			p3_print("waiting for events (pending=%d)\n", 
				 eq_poll_event_pending);
		if(timeout == PTL_TIME_FOREVER)
			twr = pthread_cond_wait(&eq_poll_cond,
						&eq_poll_cond_lock);
		else
			twr = pthread_cond_timedwait(&eq_poll_cond,
						     &eq_poll_cond_lock,
						     &ts);
		if (DEBUG_NI(d,PTL_DBG_NI_06))
			p3_print("cond wait done (pending=%d)\n",
				 eq_poll_event_pending);
	}
	if(eq_poll_event_pending > 0) {
		eq_poll_event_pending--;
		if (DEBUG_NI(d,PTL_DBG_NI_06))
			p3_print("event consumed (pending=%d)\n",
				 eq_poll_event_pending);
	}
	else {
		if(twr == ETIMEDOUT) {
			if (DEBUG_NI(d,PTL_DBG_NI_06))
				p3_print("timed out cond wait\n");
		}
		else {
			fprintf(stderr, "%lu: wtf?!\n", pthread_self());
			abort();
		}
	}
	pthread_mutex_unlock(&eq_poll_cond_lock);
#else
	if(timeout == 0)
		twr = sem_trywait(&eq_poll_semaphore);
	else if(timeout == PTL_TIME_FOREVER)
		twr = sem_wait(&eq_poll_semaphore);
	else {
		gettimeofday(&tv0, NULL);
		ts.tv_sec = tv0.tv_sec + timeout/1000;
		ts.tv_nsec = tv0.tv_usec*1000 + (timeout%1000)*1000*1000;
		...
		twr = sem_timedwait(&eq_poll_semaphore, &ts);
	}
	if(twr != 0) {
		if (DEBUG_NI(d,PTL_DBG_NI_06))
			p3_print("waiting on semaphore failed with "
				 "%d (%d: %s)\n", twr, errno, strerror(errno));
	}
	else {
		if (DEBUG_NI(d,PTL_DBG_NI_06))
			p3_print("waiting on semaphore OK\n");
	}
#endif /* EVENT_SEMAPHORE */
#endif /* !PTL_PROGRESS_THREAD */
	return PTL_OK;
