...
static 
const char usage[] = " \n\
getput_pp [-ahv] [-d <dbg>] [-m <min>] [-M <max>] \n\
          [-i [x]<inc>] [-n <cnt>] [-o <cnt>] \n\
 \n\
...
static inline
void getput(ptl_process_id_t dst_id, ptl_pt_index_t ptl, ptl_ac_index_t ace,
	    ...
	    int mbits, int mbuf, int trial, int rank, int verbose)
{
	...
	rc = PtlGetPut(msg->md_h, msg->md_h, dst_id, ptl, ace, mb, 0, 0);
	if (rc != PTL_OK) {
...
int main(int argc, char *argv[])
{
	...
	if (end_len > (ptl_size_t)ni_lim.max_getput_md) {
		end_len = ni_lim.max_getput_md;
		if (!rank) 
			printf("\nWarning: Portals NI limit exceeded; "
			       "truncating max message length to %u bytes.\n",
			       (unsigned)end_len);
	}
	...
	for (i=0; i<inflight; i++) {
		...
		if (rc != PTL_OK) {
			fprintf(stderr, "%d:PtlEQAlloc(): %s (%d)\n",
...
next_size:
	...
	while (sender && !done) {
		...
		if (i < trials) 
			getput(dst_id, ptl, ace_any, msg+m, len, src, 
			       m, i, rank, verbose);
		m += 1;
		m %= inflight;
